...
VulkanInstance::VulkanInstance() {
  ...
  if (CONSTANTS::DEBUG_MODE) {
    ATTEMPT_VK(DebugCallback::create(vk_instance, nullptr, debug_messenger));
  }
}
...
/**
 * @brief Checks if the required validation layers are supported by the Vulkan
 * instance.
 *
...
bool checkValidationLayerSupport() {
  ...
  // This loop checks if each validation layer in
  // EnvConstants::VALIDATION_LAYERS is present in the available_layers If a
  // validation layer is found, it continues to the next layer If a validation
  // layer is not found, it returns false
