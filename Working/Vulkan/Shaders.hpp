...
struct Shaders {
    ...
    inline static void loadShaders(const VulkanDevice& device) {
        ShaderModule vert(SHADER_BINARY_DIR "Shader.vert.spv", VK_SHADER_STAGE_VERTEX_BIT, &device);
        ShaderModule frag(SHADER_BINARY_DIR "Shader.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT, &device);
        add("vert", vert);
        add("frag", frag);
    }
