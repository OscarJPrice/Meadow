    ...
    command_pool(command_pool) 
{
    ...
    ATTEMPT_VK(vkAllocateCommandBuffers(command_pool.swapchain.device.vk_logical_device, 
...
void VulkanCommandBuffer::operator()(VulkanPipeline& pipeline) const {
    ...
    ATTEMPT_VK(vkBeginCommandBuffer(vk_command_buffer, &command_buffer_begin_info));
    ...
    {std::cout<< "color_attachment_info:\n" <<
        ...
        "    .clearValue:\n" <<
        "        .color:\n" <<
        "            .float32[0] = " << color_attachment_info.clearValue.color.float32[0] << "\n" <<
        "            .float32[1] = " << color_attachment_info.clearValue.color.float32[1] << "\n" <<
        "            .float32[2] = " << color_attachment_info.clearValue.color.float32[2] << "\n" <<
        "            .float32[3] = " << color_attachment_info.clearValue.color.float32[3] << "\n";}
    
    VkRenderingInfo rendering_info {
        .sType = VK_STRUCTURE_TYPE_RENDERING_INFO,
        .renderArea = {
            .offset = {0, 0},
            .extent = command_pool.swapchain.extent
        },
        .layerCount = 1,
    ...
    {std::cout<< "rendering_info:\n" <<
        ...
        "    .colorAttachmentCount = " << rendering_info.colorAttachmentCount << "\n" <<
    ...
    swapchain(swapchain) 
{
    auto queueFamilyIndices = QueueUtils::findQueueFamilies(
        swapchain.device.vk_physical_device, 
        swapchain.device.surface.vk_surface);
    ...
    ATTEMPT_VK(vkCreateCommandPool(swapchain.device.vk_logical_device, 
        &command_pool_create_info, nullptr, &vk_command_pool));
}

VulkanCommandPool::~VulkanCommandPool() {
    if (vk_command_pool != VK_NULL_HANDLE) {
        command_buffers.clear();
        vkDestroyCommandPool(swapchain.device.vk_logical_device, vk_command_pool, nullptr);
    }
}